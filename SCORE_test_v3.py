...
def connect_and_get_game_data():
    """
    Connects to the game server, waits for 'Game Start', then listens for the
    entire data stream until a termination keyword is found, and returns the full data.
    """
    ...
    game_started = False # State to track if we've seen "Game Start"
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            sock.settimeout(300)  # 5-minute timeout for listening to prevent hangs
            ...
            while True:
                ...
                # Wait for "Game Start" before we begin accumulating data
                if not game_started:
                    if "Game Start" in data_chunk:
                        print("[Game Client] ตรวจพบ 'Game Start'! เริ่มการรวบรวมข้อมูล...")
                        game_started = True
                        # Start accumulating from this chunk onwards
                        full_data_stream = data_chunk + "\n"
                    else:
                        # Ignore chunks received before the game starts (like :RY1Trig)
                        print("[Game Client] รอ 'Game Start'...")
                        continue
                else:
                    # Once game has started, accumulate all subsequent data
                    full_data_stream += data_chunk + "\n"

                # Check if the game session has ended, but only after it has started
                if game_started and any(keyword in full_data_stream for keyword in termination_keywords):
        ...
        if not game_started:
            print("[Game Client WARNING] ไม่พบ 'Game Start' ในข้อมูลที่ได้รับเลย")
            return None
        ...
        print(f"[Game Client ERROR] การเชื่อมต่อหมดเวลา (อาจเป็นตอนเชื่อมต่อ หรือระหว่างรอข้อมูล)")
        return None
...
def handle_start_process(address, *args):
    ...
    print("\n[Main] กระบวนการเสร็จสิ้นแล้ว กลับสู่สถานะรอรับคำสั่ง OSC จาก UI...")
